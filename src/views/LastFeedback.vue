  ...
  <d-container fluid class="main-content-container px-4">
    ...
    <d-row>
      <d-col lg="6" md="12" sm="12" class="mb-4">
        <bo-top-items :title="'Last Read Article'" :items="feedback"/>
      </d-col>
...
import UserRecommend from '@/components/common/ContextualHistoryRecommend.vue';
  ...
  mounted() {
    ...
    axios.get('/api/dashboard/config')
      .then((response) => {
        ...
        this.feedbackTypes = response.data.recommend.data_source.read_feedback_types;
        const requests = [];
        this.feedbackTypes.forEach((feedbackType) => {
          requests.push(axios.get(`/api/dashboard/user/${this.user_id}/lastfeedback/${feedbackType}`));
        });
